...
-- category_list+count 
select c.id, c.name, count(*) as count from category c 
inner join product p
on p.category_id = c.id
group by p.category_id;

-- main list
select @ROWNUM:=@ROWNUM+1 AS ROWNUM, dt.* from ( 
	...
	from product p
	right outer join display_info d
	on p.id = d.product_id
	left outer join product_image pm
	on p.id = pm.product_id
	left outer join file_info f
	on f.id = pm.file_id
	where pm.type="ma" and p.category_id like '%'
	order by p.id) AS dt
    ,(
		select @ROWNUM := 0
	) tmp
...
where pi.product_id in (
	select product_id from display_info where id = 2
    );
...
from product_price
where product_id in (
	select product_id from display_info where id =1
    );
...
-- comments - list
 select 
	...
	rc.reservation_info_id as reservation_info_id,
	rc.score as score,
...
where reservation_info_id in (
	select id from reservation_info where display_info_id= 1);
